...
function getNetSalary(salary, personalRelief, insuranceRelief, pensionFund, housingRel, ownerOccupier, disabilityExem, otherPension) {
  const tax = getPayAsYouEarTax(salary, personalRelief, insuranceRelief, pensionFund, housingRel, ownerOccupier, disabilityExem)
  const nhif = nhifDeduction(salary)
  const nssf = nssfDeduction(salary, otherPension)


  console.log("Net Salary: " + (salary - tax - nhif - nssf))
...
getNetSalary(5000000, 1000, 30000, 20000, 1000, 6000, 200000, 0)